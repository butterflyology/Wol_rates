...
car <- weinDat[weinDat$Bacterium == "Cardinium" & weinDat$Class == "insecta" & weinDat$Infected <= weinDat$Total& weinDat$Pool_Size ==1,]
...
fit <- stan(file = "hammModMultLev.stan", data = carDat, iter = 5000, chains = 4, seed = 4, pars=c('thetaFRaw','thetaSRaw','sigmaRaw'), include=FALSE) 
...
infectNew <- t(extract(fit, pars="infectNew")$infectNew)
