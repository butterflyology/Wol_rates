...
pPlot1 <-function (formula, data, line.fun = mean, pal = "appletv", back.col = gray(1), 
                  ...
                  add = F, sortx = T, y.levels = NULL, bar.border.col = NULL, 
                  cex.lab = 1, cex.axis = 1, ...) 
{
  ...
  if (is.null(ylim) == TRUE & is.null(y.levels) == TRUE) {
    steps.p <- c(seq(0.001, 0.01, 0.001), seq(1e-04, 0.001, 
                                              0.001), seq(1e-05, 1e-04, 1e-04), seq(1e-06, 1e-05, 
                                                                                    1e-05), seq(1e-07, 1e-06, 1e-06), seq(1e-08, 1e-07, 
                                                                                                                          1e-07), seq(1e-09, 1e-08, 1e-08), 1, 2, 5, 10, 25, 
                 50, 100, seq(100, 1000, 100), seq(1000, 10000, 1000), 
                 seq(10000, 1e+05, 10000), seq(1e+05, 1e+06, 1e+05), 
                 seq(1e+06, 1e+07, 1e+06), seq(1e+07, 1e+08, 1e+07), 
                 seq(1e+08, 1e+09, 1e+08))
    range <- max(dv.v) - min(dv.v)
    steps.p.m <- range/steps.p
    best.step.size <- min(steps.p[which(abs(steps.p.m - 10) == 
                                          min(abs(steps.p.m - 10)))])
    plot.min <- floor(min(dv.v)/best.step.size) * best.step.size
    plot.max <- ceiling((max(dv.v) - plot.min)/best.step.size) * 
      best.step.size
  ...
  if (n.iv == 2) {
    ...
    mtext(text = paste(names(bean.mtx)[2], "=", unique(bean.mtx[, 
                                                                2])), side = 1, line = 2, at = text.loc, cex = cex.lab)
  }
}
