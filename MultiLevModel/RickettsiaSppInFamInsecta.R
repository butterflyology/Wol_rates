...
fit <- stan(file = "hammModMultLev.stan", data = rickDat, iter = 1000, chains = 4, seed = 4, pars=c('thetaFRaw','thetaSRaw','sigmaRaw'), include=FALSE) 
...
par(mfrow=c(1,3))
par(mar = c(3, 4, 3, 0.3))
plot(1:735, medS, ylim=c(0,1), type="n", main="Species Level",ylab="theta",las=1)
for(i in 1:nrow(thetaS)) {
  lines(rep(i,2), c(upperS[i],lowerS[i]))
}
points(1:735,medS, col="red", pch=".")
